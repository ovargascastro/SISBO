...
import logic.AbaaTbPermisos;
import logic.AbaaTbPersona;
import logic.AbaaTbRol;
...
public class usuariosDAO {
    ...
    private AbaaTbRol rol(ResultSet rs) {
        try {
            AbaaTbRol ob = new AbaaTbRol();
            ob.setRolIdPk(rs.getInt("Rol_id_PK"));
            ob.setRolDesc(rs.getString("Rol_desc"));
            return ob;
        } catch (SQLException ex) {
            return null;
        }
    ...
    private AbaaTbRolxPermiso rolPerm(ResultSet rs) {
        try {
            AbaaTbRolxPermiso ob = new AbaaTbRolxPermiso();
            ob.setRol_x_Perm_id_PK(rs.getInt("Rol_x_Perm_id_PK"));
            ob.setPerm_id_FK(rol(rs));
            ob.setRol_id_FK(permiso(rs));
            return ob;
        } catch (SQLException ex) {
            return null;
        }
    ...
    private AbaaTbPermisos permiso(ResultSet rs) {
        try {
            AbaaTbPermisos ob = new AbaaTbPermisos();
            ob.setPermIdPk(rs.getInt("Perm_id_PK"));
            ob.setPermDesc(rs.getString("Perm_desc"));
            return ob;
        } catch (SQLException ex) {
            return null;
        }
    ...
    public List<AbaaTbRolxPermiso> getRoles() throws Exception {

        List<AbaaTbRolxPermiso> resultado = new ArrayList<AbaaTbRolxPermiso>();
        try {
            String sql = "select * from ABAA_TB_RolxPermiso rp\n"
                    + "inner join ABAA_TB_Rol r on rp.Rol_id_FK=r.Rol_id_PK\n"
                    + "inner join ABAA_TB_Permiso pe on rp.Perm_id_FK=pe.Perm_id_PK;";
            sql = String.format(sql);
            ResultSet rs = db.executeQuery(sql);
            while (rs.next()) {
                resultado.add(rolPerm(rs));
            }
        } catch (SQLException ex) {
        }
        return resultado;
    ...
    public void InsertarUsuario(AbaaTbUsuario objeto) throws Exception {
        ...
        preparedStmt.setInt(4,objeto.getPersona().getRolAux());
